...
Hooks.once('init', async function() {
  ...
  Actors.registerSheet("reclaimboardgame", ReclaimBGActorSheet, { makeDefault: true });
  Items.unregisterSheet("core", ItemSheet);
  Items.registerSheet("reclaimboardgame", ReclaimBGItemSheet, { makeDefault: true });
...
async function createItemMacro(data, slot) {
  ...
  const command = `game.reclaimboardgame.rollItemMacro("${item.name}");`;
  ...
  if (!macro) {
    macro = await Macro.create({
      ...
      img: item.img,
      command: command,
      flags: { "reclaimboardgame.itemMacro": true }
    });
  }
